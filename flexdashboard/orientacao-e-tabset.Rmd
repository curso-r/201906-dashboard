...
```{r, fig.height=4, fig.width=7}
pkmn %>%
  filter(tipo_1 == "gelo", !is.na(id_geracao)) %>% 
  mutate(
    geracao = paste("Geração ", id_geracao)
  ) %>%
  ggplot(aes(x = geracao)) + 
  geom_bar(fill = cores["gelo"]) + 
  labs(x = "", y = "Número de pokémon") +
  theme_minimal()
...
2. Acrescente mais uma [aba](https://www.curso-r.com) à segunda linha, replicando os gráficos para algum outro tipo.
